...
def learn(env, policy_func, discriminator, expert_dataset,
    ...
    var_list = [v for v in all_var_list if v.name.split(
        "/")[1].startswith("pol")]
    vf_var_list = [v for v in all_var_list if v.name.split(
        "/")[1].startswith("vf")]
    ...
    for shape in shapes:
        ...
        start += sz
    gvp = tf.add_n([U.sum(g * tangent) for (g, tangent)
                    in zipsame(klgrads, tangents)])  # pylint: disable=E1111
    ...
    compute_vflossandgrad = U.function(
        [ob, ret], U.flatgrad(vferr, vf_var_list))
    ...
    seg_gen = traj_segment_generator(
        pi, env, discriminator, timesteps_per_batch, stochastic=True)
    ...
    while True:
        ...
        if rank == 0:
            ...
            # g_loss_stats.add_all_summary(writer, newbuffer, iters_so_far)
            d_loss_stats.add_all_summary(
                writer, np.mean(d_losses, axis=0), iters_so_far)
